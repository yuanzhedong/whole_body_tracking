"""Pipeline diagram for BONES-SEED -> UniMoTok VAE -> BFM-Zero sim2sim.

Same Stage-1 flow as the HoloMotion pipeline, but validated with BFM-Zero, which
additionally executes near-ground motion (crouch/sit/squat) that HoloMotion fails.
Writes pipeline_bfm.png.
"""
import os
import matplotlib
matplotlib.use("Agg")
...
DATA = "#4a78b5"
VAE = "#7a4fb5"
VAL = "#3c9a5f"   # BFM-Zero validator (green — handles near-ground)
GREY = "#555555"
...
ax.text(6.0, 7.5, "Stage 1 (BFM-Zero validator) — learn a G1 motion latent, validate it in physics",
...
# Row B — BFM-Zero validator
box(ax, 11.3, 3.75, 3.4, 1.05, "decode → qpos_36\n→ pose_aa (robot a-a)\n(reference motion)", DATA, fc="#eaf1fb", fs=9.6)
arrow(ax, 9.6, 3.75, 8.5, 3.75, label="encode → z")
box(ax, 6.75, 3.75, 3.3, 1.05, "BFM-Zero\nForward-Backward\nfoundation model", VAL, fc="#e9f4ee", fs=9.8, bold=True)
arrow(ax, 5.1, 3.75, 4.05, 3.75, label="actions")
box(ax, 2.3, 3.75, 3.2, 1.05, "MuJoCo G1\nphysics rollout", VAL, fc="#e9f4ee", bold=True)
...
# near-ground advantage callout on the validator (below the BFM-Zero box)
ax.add_patch(FancyArrowPatch((6.75, 2.85), (6.75, 3.22), arrowstyle="-|>",
             mutation_scale=14, linewidth=1.6, linestyle=(0, (4, 3)), color=VAL, zorder=1))
ax.text(6.75, 2.55, "also executes near-ground motion\n(crouch / sit / squat) that HoloMotion fails",
        ha="center", va="center", fontsize=8.6, style="italic", color=VAL)
...
ax.text(8.2, 0.7, "■", color=VAL, fontsize=13); ax.text(8.55, 0.7, "physics validator (BFM-Zero + MuJoCo)", fontsize=9, va="center")

fig.tight_layout()
out = f"{HERE}/pipeline_bfm.png"
