...
VAE=$(for f in $(ls -t UniMoTok/experiments/biomechanics_tokenizer/EX_gated8/checkpoints/epoch=*.ckpt); do [ "$(stat -c %s $f)" -gt 40000000 ] && echo "$f" && break; done)
NORM=stage2/out/g1_dataset_T4within/normalization.npz
L=logs/rsl_rl/g1_flat; G=4; LOG=stage2/out/gated_sim2sim.log; : > $LOG
...
trk(){ CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=$G OMNI_KIT_ACCEPT_EULA=YES OMNI_USER_HOME=/tmp/omni_gs2_$3 timeout 500 .venv/bin/python -u stage2/bench_earlyfreeze.py --motion "$1" --teacher_ckpt "$2" --no_freeze --late --steps 300 2>&1 | grep -oE "final survival = [0-9.]+" | grep -oE "[0-9.]+$"; }
clip(){ local c=$1 teach=$2 tag=$3
  local DS=stage2/out/g1_gs2_$tag; rm -rf $DS; mkdir -p $DS/val; cp $NORM $DS/normalization.npz
  ...
  trunc artifacts/$c:v0/motion.npz /tmp/gs2o_$tag.npz; trunc $dec /tmp/gs2d_$tag.npz
  local so=$(trk /tmp/gs2o_$tag.npz "$teach" o$tag); local sd=$(trk /tmp/gs2d_$tag.npz "$teach" d$tag)
